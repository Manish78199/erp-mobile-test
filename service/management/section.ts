...
const get_class_section = async (class_id: string) => {
    try {
        const response = await axios.get(`${ApiRoute?.section.get_class_section}/${class_id}`, { headers: get_headers() })
...
const assign_section_stdent = async (section_id: string, students: string[]) => {
    return axios.patch(`${ApiRoute?.section.create}/${section_id}/students`, students, { headers: get_headers() })
}
