...
//       if (token) {
//         const role = await roleAuth(token)
...
export default function Login() {
  ...
  const [userRole, setUserRole] = useState<"STUDENT" | "MANAGEMENT">("STUDENT")
  const [requesting, setRequesting] = useState(false)
  const [showPassword, setShowPassword] = useState(false)
  const [loading, setLoading] = useState(true)
  const [checked, setChecked] = useState(false)
  ...
  const primaryColor = isStudent ? "#5B7FE5" : "#6366F1"
  const accentColor = isStudent ? "#3B82F6" : "#4F46E5"
  const roleTitle = isStudent ? "Student Login" : "Management Login"
  const roleSubtitle = isStudent ? "Access your learning portal" : "Access management dashboard"
    ...
    <SafeAreaView className="flex-1">
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} className="flex-1">
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          ...
          <View style={{ backgroundColor: primaryColor }} className="flex-1">
            {/* Header with Role Toggle */}
            <View className="px-6 mt-3 pt-6 pb-4">
              <Typography className="text-white text-sm font-medium opacity-80 mb-4">Select Login Type</Typography>

              <View className="flex-row bg-white bg-opacity-20 rounded-full p-1 gap-2">
                <TouchableOpacity
                  onPress={() => setUserRole("STUDENT")}
                  className={`flex-1 py-3 rounded-full flex-row items-center justify-center gap-2 transition-all ${
                    isStudent ? "bg-indigo-500" : "bg-transparent"
                  }`}
                >
                  <Ionicons name="school" size={18} color={isStudent ? "white" : "gray"} />
                  <Typography className={`font-semibold text-sm ${isStudent ? "text-white" : "text-gray"}`}>
                    Student
                  </Typography>
                </TouchableOpacity>

                <TouchableOpacity
                  onPress={() => setUserRole("MANAGEMENT")}
                  className={`flex-1 py-3 rounded-full flex-row items-center justify-center gap-2 transition-all ${
                    !isStudent ? "bg-indigo-500" : "bg-transparent"
                  }`}
                >
                  <Ionicons name="briefcase" size={18} color={!isStudent ? "white" : "gray"} />
                  <Typography className={`font-semibold text-sm ${!isStudent ? "text-white" : "text-gray"}`}>
                    Management
                  </Typography>
                </TouchableOpacity>
            ...
            <View className="items-center py-8">
              <Image source={loginImage} className="w-48 h-32" resizeMode="contain" />
            </View>
            ...
            <View className="flex-1 mt-4">
              <View className="bg-white rounded-t-3xl flex-1 px-6 pt-8 shadow-lg">
                {/* Dynamic Title */}
                <Typography className="text-3xl font-bold text-gray-900 mb-2 text-balance">{roleTitle}</Typography>
                <Typography className="text-gray-500 text-base mb-8">{roleSubtitle}</Typography>

                {/* Form */}
                <View>
                  {/* School Code Input */}
                  <View className="mb-5">
                    <Typography className="text-gray-700 text-sm mb-2 font-semibold">School Code</Typography>
                    ...
                    {errors.school_code && touched.school_code && (
                      <Typography className="text-red-500 text-xs mt-2 font-medium">{errors.school_code}</Typography>
                    )}
                  </View>

                  {/* User ID Input */}
                  <View className="mb-5">
                    <Typography className="text-gray-700 text-sm mb-2 font-semibold">User ID</Typography>
                    <View className="flex-row items-center border border-gray-200 rounded-xl bg-gray-50 px-4 py-3">
                    ...
                    {errors.userId && touched.userId && (
                      <Typography className="text-red-500 text-xs mt-2 font-medium">{errors.userId}</Typography>
                    )}
                  </View>

                  {/* Password Input */}
                  <View className="mb-6">
                    <Typography className="text-gray-700 text-sm mb-2 font-semibold">Password</Typography>
                    <View className="flex-row items-center border border-gray-200 rounded-xl bg-gray-50 px-4 py-3">
                    ...
                    {errors.password && touched.password && (
                      <Typography className="text-red-500 text-xs mt-2 font-medium">{errors.password}</Typography>
                    )}
                  </View>
                  ...
                  {requesting ? (
                    <TouchableOpacity
                      disabled={true}
                      className="rounded-xl py-4 mb-6 flex-row items-center justify-center bg-gray-300"
                    >
                      <ActivityIndicator size="small" color="white" />
                      <Typography className="text-white font-semibold text-base ml-2">Logging in...</Typography>
                    </TouchableOpacity>
                  ) : (
                    <TouchableOpacity
                      onPress={() => handleSubmit()}
                      style={{ backgroundColor: primaryColor }}
                      className="rounded-xl py-4 mb-6 flex-row items-center justify-center shadow-md"
                    >
                      <Typography className="text-white font-bold text-base mr-2">Login</Typography>
                      <Ionicons name="arrow-forward" size={20} color="white" />
                    </TouchableOpacity>
                  )}
                </View>
                ...
                {/* Footer */}
                <View className="items-center pb-6 border-t border-gray-100 pt-6">
                  <Typography className="text-gray-400 text-xs">© 2025 VEDATRON. All rights reserved.</Typography>
                </View>
              </View>
            </View>
          </View>
