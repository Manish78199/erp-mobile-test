...
//             <Typography className="text-lg font-semibold text-gray-900">{item.registration}</Typography>
...
//                   <MaterialCommunityIcons name={getStatusIcon(item.status) as any} size={14} />
//                   <Typography className="text-xs font-medium capitalize">{item.status}</Typography>
//                 </View>
//               </View>
//               {item.status === "emergency" && (
//                 <View className="px-2 py-1 rounded bg-red-100">
//                   <Typography className="text-xs font-medium text-red-800">EMERGENCY</Typography>
//                 </View>
//               )}
//             </View>
//           </View>
...
//           <Typography className="text-xs text-gray-600">Last Update</Typography>
//           <Typography className="text-sm font-medium text-gray-900">{getTimeSinceUpdate(item.lastUpdate)}</Typography>
//         </View>
//       </View>
...
//           <Typography className="text-sm font-medium text-gray-900 flex-1">{item.location.address}</Typography>
//         </View>
//         <View className="flex-row gap-4">
//           <View className="flex-row items-center gap-1">
//             <MaterialCommunityIcons name="run" size={14} color="#666" />
//             <Typography className="text-xs text-gray-600">Speed:</Typography>
//             <Typography className="text-xs font-medium text-gray-900">{item.speed} km/h</Typography>
//           </View>
//           <View className="flex-row items-center gap-1">
//             <MaterialCommunityIcons name="navigation" size={14} color="#666" />
//             <Typography className="text-xs text-gray-600">Heading:</Typography>
//             <Typography className="text-xs font-medium text-gray-900">{item.heading}°</Typography>
//           </View>
//         </View>
//       </View>
...
//             <Typography className="text-sm text-gray-600">Students on Board</Typography>
//           </View>
...
//             <Typography className="text-xs text-gray-600">Fuel</Typography>
//           </View>
//           <Typography className={`text-sm font-medium ${getFuelLevelColor(item.fuelLevel)}`}>{item.fuelLevel}%</Typography>
//         </View>

//         <View className="flex-1 p-3 bg-gray-50 rounded-lg">
//           <View className="flex-row items-center gap-1 mb-1">
//             <MaterialCommunityIcons name="thermometer" size={14} color={getTemperatureColor(item.temperature)} />
//             <Typography className="text-xs text-gray-600">Temp</Typography>
//           </View>
//           <Typography className={`text-sm font-medium ${getTemperatureColor(item.temperature)}`}>{item.temperature}°C</Typography>
//         </View>
...
//             />
//             <Typography className="text-xs text-gray-600">Engine</Typography>
//           </View>
//           <Typography className={`text-sm font-medium ${item.engineStatus === "on" ? "text-green-600" : "text-gray-600"}`}>
...
//         <View className="mb-4">
//           <View className="flex-row items-center justify-between mb-2">
//             <Typography className="text-sm text-gray-600">Trip Progress</Typography>
//             <Typography className="text-sm font-medium text-gray-900">{item.tripProgress}%</Typography>
//           </View>
//           <View className="h-2 bg-gray-200 rounded-full overflow-hidden">
//             <View className="h-full bg-green-500" style={{ width: `${item.tripProgress}%` }} />
//           </View>
//           <View className="flex-row items-center justify-between mt-2">
//             <Typography className="text-xs text-gray-600">Next: {item.nextStop}</Typography>
//             <Typography className="text-xs font-medium text-gray-900">ETA: {item.eta}</Typography>
//           </View>
//         </View>
//       )}
...
//             <Typography className="text-xs font-medium text-blue-600">View Map</Typography>
//           </View>
//         </Link>
...
//             <Typography className="text-xs font-medium text-blue-600">History</Typography>
//           </View>
//         </TouchableOpacity>
//         {item.status === "emergency" && (
...
//               <Typography className="text-xs font-medium text-white">Respond</Typography>
//             </View>
//           </TouchableOpacity>
//         )}
//       </View>
...
//                 <Typography className="text-lg font-bold text-foreground">Vehicle Tracking</Typography>
//               </View>
//         {/* Header */}
//         <View className="px-4 py-6">
//           <Typography className="text-2xl font-bold text-gray-900">GPS Tracking</Typography>
//           <Typography className="text-gray-600 mt-2">Real-time vehicle location and status monitoring</Typography>

//           {/* Auto-refresh */}
//           <View className="flex-row items-center justify-between mt-4">
//             <View className="flex-row items-center gap-2">
//               <Typography className="text-sm text-gray-600">Auto-refresh:</Typography>
//               <TouchableOpacity
...
//               </TouchableOpacity>
//             </View>
//             <Typography className="text-xs text-gray-600">Last: {lastRefresh.toLocaleTimeString()}</Typography>
//           </View>
//         </View>
...
//                   <Typography className={`text-lg font-bold ${item.textColor}`}>
//                     {filteredVehicles.filter((v) => v.status === item.status).length}
//                   </Typography>
//                   <Typography className="text-xs text-gray-600 capitalize">{item.status}</Typography>
//                 </View>
//               ))}
//             </View>
...
import { useEffect, useState } from "react"
import { View, ScrollView, TouchableOpacity, TextInput, FlatList, ActivityIndicator, Alert } from "react-native"
import { SafeAreaView } from "react-native-safe-area-context"
import { MaterialCommunityIcons } from "@expo/vector-icons"
import { Link, useRouter } from "expo-router"
import { get_vehicle } from "@/service/management/transport"
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      try {
        ...
        const mappedVehicles: VehicleLocation[] = fetchedVehicles.map((vehicle: any) => ({
          _id: vehicle._id || Math.random().toString(),
          registration: vehicle.vehicle_number || "N/A",
          driver: vehicle.driver || "Unassigned Driver",
          route: vehicle.route || "Route Not Assigned",
          status: vehicle.gpsEnabled ? getRandomStatus() : "offline",
          location: {
            address: vehicle.location || "Location Unavailable",
            coordinates: {
              lat: vehicle.latitude || 13.0843,
              lng: vehicle.longitude || 80.2705,
            },
          },
          speed: vehicle.currentSpeed || getRandomSpeed(),
          heading: vehicle.heading || Math.floor(Math.random() * 360),
          studentsOnBoard: vehicle.currentOccupancy || Math.floor(Math.random() * (vehicle.capacity || 35)),
          maxCapacity: vehicle.capacity || 35,
          fuelLevel: vehicle.fuelLevel || Math.floor(Math.random() * 100),
          engineStatus: vehicle.engineStatus || (Math.random() > 0.3 ? "on" : "off"),
          temperature: vehicle.engineTemperature || 65 + Math.floor(Math.random() * 30),
          lastUpdate: vehicle.lastUpdate || new Date().toISOString(),
          nextStop: vehicle.nextStop || "Determining Route",
          eta: vehicle.eta || `${Math.floor(Math.random() * 30) + 5} min`,
          distanceToDestination: vehicle.distanceRemaining || Number((Math.random() * 10).toFixed(1)),
          tripProgress: vehicle.tripProgress || Math.floor(Math.random() * 100),
        }))

        setVehicleLocations(mappedVehicles)
        setLastRefresh(new Date())
      } catch (error) {
        Alert.alert("Error", "Failed to fetch vehicle tracking data. Please try again.")
        console.error("[v0] GPS Tracking fetch error:", error)
      } finally {
        setLoading(false)
      }
    }
    ...
    >
      ...
      <View className="flex-row gap-3 mb-4">
        <View className="flex-1 p-3 bg-gray-50 rounded-lg">
          ...
          <Typography className={`text-sm font-medium ${getFuelLevelColor(item.fuelLevel)}`}>
            {item.fuelLevel}%
          </Typography>
        </View>
        ...
        <View className="flex-1 p-3 bg-gray-50 rounded-lg">
          ...
          <Typography className={`text-sm font-medium ${getTemperatureColor(item.temperature)}`}>
            {item.temperature}°C
          </Typography>
        </View>
        ...
        <View className="flex-1 p-3 bg-gray-50 rounded-lg">
          ...
          <Typography
            className={`text-sm font-medium ${item.engineStatus === "on" ? "text-green-600" : "text-gray-600"}`}
          >
            {item.engineStatus.toUpperCase()}
          </Typography>
        </View>
      ...
      <ScrollView className="flex-1">
        ...
        {loading ? (
          ...
          <View className="items-center justify-center py-12 px-4">
            ...
            <Typography className="text-gray-500 mt-4 text-center text-lg font-medium">No vehicles found</Typography>
            <Typography className="text-gray-400 mt-2 text-center text-sm">
              {searchTerm || filterStatus !== "all"
                ? "Try adjusting your search or filters"
                : "No vehicles with GPS tracking available"}
            </Typography>
          </View>
        )}
      </ScrollView>
