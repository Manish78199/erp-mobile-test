  ...
  return (
    <ScrollView className="flex-1 bg-[#F0F4F8]" showsVerticalScrollIndicator={false}>
      ...
      <View className="flex-row items-center justify-between bg-[#6A5ACD] pt-12 pb-5 px-4 rounded-b-[25px]">
        ...
        <Typography className="text-xl font-bold text-white">Hostel Information</Typography> 
      ...
      <View className="px-4 -mt-8 mb-5">
        <View className="bg-white rounded-2xl p-4 shadow-lg elevation-5">
          <View className="flex-row items-center justify-between mb-4">
            <Typography className="text-lg font-bold text-[#2C3E50]">My Room</Typography> 
            <View className="bg-[#6A5ACD20] px-3 py-1 rounded-xl">
              <Typography className="text-sm font-bold text-[#6A5ACD]">{hostelData.room.number}</Typography> 
            </View>
          </View>
          ...
          <View className="flex-row justify-between mb-4">
            <View className="items-center flex-1">
              ...
              <Typography className="text-sm font-semibold text-[#2C3E50] mt-1">{hostelData.room.block}</Typography> 
              <Typography className="text-xs text-[#7F8C8D]">Block</Typography> 
            </View>
            <View className="items-center flex-1">
              <Icon name="layers" size={24} color="#00BCD4" />
              <Typography className="text-sm font-semibold text-[#2C3E50] mt-1">{hostelData.room.floor}</Typography> 
              <Typography className="text-xs text-[#7F8C8D]">Floor</Typography> 
            </View>
            <View className="items-center flex-1">
              <Icon name="people" size={24} color="#2ECC71" />
              <Typography className="text-sm font-semibold text-[#2C3E50] mt-1">{hostelData.room.type}</Typography> 
              <Typography className="text-xs text-[#7F8C8D]">Type</Typography> 
            </View>
          </View>
          ...
          <View className="bg-[#EAECEE] rounded-xl p-3">
            <View className="flex-row items-center justify-between">
              <View>
                <Typography className="text-sm font-semibold text-[#2C3E50]">Warden: {hostelData.room.warden}</Typography> 
                <Typography className="text-xs text-[#7F8C8D]">{hostelData.room.wardenContact}</Typography> 
              </View>
      ...
      <View className="flex-row px-4 mb-5">
        {["room", "facilities", "rules", "maintenance"].map((tab) => (
          ...
          >
            <Typography className={`text-xs font-semibold ${selectedTab === tab ? "text-white" : "text-[#7F8C8D]"}`}>
      ...
      {selectedTab === "room" && (
        <View className="p-4">
          <Typography className="text-xl font-bold text-[#2C3E50] mb-4">Roommates</Typography> 
              ...
              <View key={roommate.id} className="bg-white rounded-2xl p-4 shadow-lg elevation-5">
                <View className="flex-row items-center justify-between">
                  <View className="flex-row items-center flex-1">
                    ...
                    <View>
                      <Typography className="text-base font-semibold text-[#2C3E50]">{roommate.name}</Typography> 
                      <Typography className="text-sm text-[#7F8C8D]">{roommate.class}</Typography> 
                      <Typography className="text-xs text-[#7F8C8D]">From {roommate.hometown}</Typography> 
                    </View>
                  </View>
                  <TouchableOpacity className="bg-[#2ECC71] p-2 rounded-lg">
      ...
      {selectedTab === "facilities" && (
        <View className="p-4">
          <Typography className="text-xl font-bold text-[#2C3E50] mb-4">Room Facilities</Typography> 
          <View className="bg-white rounded-2xl p-4 shadow-lg elevation-5">
              ...
              {hostelData.facilities.map((facility, index) => (
                <View key={index} className="w-1/2 mb-4">
                  <View className="flex-row items-center">
                    ...
                    <Typography className="text-sm text-[#2C3E50] font-medium">{facility.name}</Typography> 
                  </View>
                </View>
              ))}
            </View>
      ...
      {selectedTab === "rules" && (
        <View className="p-4">
          <Typography className="text-xl font-bold text-[#2C3E50] mb-4">Hostel Rules</Typography> 
          <View className="bg-white rounded-2xl p-4 shadow-lg elevation-5">
            {hostelData.rules.map((rule, index) => (
              <View key={index} className="flex-row items-start mb-3 last:mb-0">
                <View className="w-6 h-6 rounded-full bg-[#6A5ACD20] items-center justify-center mr-3 mt-0.5">
                  <Typography className="text-xs font-bold text-[#6A5ACD]">{index + 1}</Typography> 
                </View>
                <Typography className="text-sm text-[#2C3E50] leading-5 flex-1">{rule}</Typography> 
              </View>
            ))}
          </View>
        </View>
      ...
      {selectedTab === "maintenance" && (
        <View className="p-4">
          <View className="flex-row justify-between items-center mb-4">
            <Typography className="text-xl font-bold text-[#2C3E50]">Maintenance Requests</Typography> 
            <TouchableOpacity className="bg-[#6A5ACD] px-4 py-2 rounded-xl">
              <Typography className="text-sm font-semibold text-white">Report Issue</Typography> 
            </TouchableOpacity>
          </View>
          <View className="gap-3">
            {hostelData.maintenance.map((request) => (
              <View key={request.id} className="bg-white rounded-2xl p-4 shadow-lg elevation-5">
                <View className="flex-row justify-between items-center mb-2">
                  <Typography className="text-base font-semibold text-[#2C3E50] flex-1">{request.issue}</Typography> 
                  <View
                ...
                <View className="flex-row justify-between items-center">
                  <View>
                    <Typography className="text-xs text-[#7F8C8D]">Reported: {formatDate(request.reportedDate)}</Typography> 
                    {request.resolvedDate && (
                      <Typography className="text-xs text-[#2ECC71]">Resolved: {formatDate(request.resolvedDate)}</Typography> 
                    )}
                  </View>
                  <View
                    className="px-2 py-1 rounded-lg"
